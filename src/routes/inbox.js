...
router.post('/fetch', async (req, res, next) => {
  try {
    ...
    res.json({ channel: type, count: messages.length, messages });
  } catch (err) { next(err); }
});
...
router.post('/triage', async (req, res, next) => {
  try {
    ...
    res.json({ messageId: message.id, ...result });
  } catch (err) { next(err); }
});
...
router.post('/draft', async (req, res, next) => {
  try {
    ...
    res.json({ messageId: message.id, draft });
  } catch (err) { next(err); }
});
...
router.post('/process', async (req, res, next) => {
  try {
    ...
    res.json({ channel: type, processed: results.length, results });
  } catch (err) { next(err); }
});
...
router.post('/archive', async (req, res, next) => {
  try {
    ...
    res.json({ channel: type, messageId, ...result });
  } catch (err) { next(err); }
});
